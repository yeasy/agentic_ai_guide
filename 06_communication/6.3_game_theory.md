...
当 Agent 的数量从几个增加到几百个、几千个时，我们不再关注"个体"的角色，而是关注"群体"涌现出的智能。这被称为 **Swarm Intelligence（群体智能）**。
...
- **局部交互**：每个 Agent 只遵循简单的局部规则（Handoffs），例如"如果我处理不了这个问题，就把它交给 Sales Agent"
...
**Red Teaming** 是一种利用博弈对抗提升系统安全性的方法：
- **Blue Agent**：负责防御系统，检测和阻止恶意输入
- **Red Agent**：负责尝试注入恶意 Prompt、绕过安全机制
