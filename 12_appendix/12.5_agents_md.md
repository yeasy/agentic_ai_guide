...
AGENTS.md 是一个由 OpenAI 发起的开放标准，旨在为 AI 编程智能体提供项目级的指令和上下文。它于 2025 年 12 月被捐赠给 Linux Foundation 的 Agentic AI Foundation (AAIF)。

### 12.5.1 什么是 AGENTS.md
...
> - `AGENTS.md` → AI 编程智能体的工作指南

### 12.5.3 为什么需要 AGENTS.md
...
mindmap
    root((project-root/))
        ...
        AGENTS.md["AGENTS.md<br>(给 AI 智能体看的工作指南)"]
        src/
            AGENTS_SUB["AGENTS.md<br>(可选: 子目录特定指令)"]
        tests/
            AGENTS_TEST["AGENTS.md<br>(可选: 测试目录特定指令)"]
```

图 12-2：AGENTS.md 项目布局结构 (AGENTS.md Project Layout Structure)
...
不要试图把所有信息都塞进一个 `AGENTS.md`。这会消耗 **词元 (Token)** 并降低智能体的注意力。
...
mindmap
    root((project-root/))
        AGENTS.md["AGENTS.md<br>(通用智能体指令)"]
        CLAUDE.md["CLAUDE.md<br>(Claude Code 专属配置)"]
        .cursor/
            rules["rules<br>(Cursor 专属配置)"]
        src/
            AGENTS_SUB["AGENTS.md<br>(子目录特定指令)"]
```

图 12-3：AGENTS.md 与 CLAUDE.md 共存结构 (Coexistence Structure of AGENTS.md and CLAUDE.md)
