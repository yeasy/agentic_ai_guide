...
传统的 LLM 采用"System 1"思维模式——快速、直觉性的响应。2024 年底开始，OpenAI 和 Anthropic 推出的推理模型引入了"System 2"思维——慢速、深思熟虑的推理过程，为 Agent 的决策能力带来了质的飞跃。
...
**核心机制**：
- 模型使用内在的思维链（Chain of Thought）进行推理
- 通过强化学习优化思维过程
- 学会识别和纠正自己的错误
- 将复杂步骤分解为简单步骤
- 当前方法不奏效时尝试不同途径
...
推理模型引入了一种新的安全对齐方式——在推理过程中应用安全约束。
...
│         ┌─────────────┐ ┌─────────────┐ ┌─────────────┐│
│         │  常规任务    │ │  复杂推理   │ │  编程调试   ││
│         │  (GPT-4o)   │ │(o3-mini-high)│ │(o3-mini-med)││
│         └─────────────┘ └─────────────┘ └─────────────┘│
└─────────────────────────────────────────────────────────┘
```
...
3. **安全对齐**：通过 Deliberative Alignment 提升安全性
4. **灵活配置**：可调节推理深度实现成本与质量平衡
5. **Agent 应用**：在复杂决策、代码审查、数学证明等场景价值显著
