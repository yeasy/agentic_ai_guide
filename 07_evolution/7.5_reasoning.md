...
传统的 LLM 采用 **系统 1 (System 1)** 思维模式——快速、直觉性的响应。2024 年底开始，OpenAI 和 Anthropic 推出的推理模型引入了 **系统 2 (System 2)** 思维——慢速、深思熟虑的推理过程，为智能体的决策能力带来了质的飞跃。
...
图 7-3：思维链 (Chain of Thought) 推理流程 (Chain of Thought Reasoning Process)

**核心机制**：

- 模型使用内在的 **思维链（Chain of Thought）** 进行推理
- 通过强化学习优化思维过程
- 学会识别和纠正自己的错误
- 将复杂步骤分解为简单步骤
- 当前方法不奏效时尝试不同途径
...
推理模型引入了一种新的安全对齐方式——**审慎对齐（Deliberative Alignment）**——在推理过程中应用安全约束。
...
图 7-4：审慎对齐机制 (Deliberative Alignment Mechanism)
...
class ReasoningAgent:
    ...
    def routine_action(self, task: str) -> str:
        """常规任务使用快速模型"""
        return self.gpt4o.chat.completions.create(
            model="gpt-4o",
            messages=[{"role": "user", "content": task}]
        )
```
...
graph TD
    subgraph DecisionLayer [智能体决策层]
        Router[路由层 (GPT-4o)] --> Classifier[任务分类器]
        Classifier -->|常规任务| Routine[常规任务 (GPT-4o)]
        Classifier -->|复杂推理| Complex[复杂推理 (o3-mini-high)]
        Classifier -->|编程调试| Coding[编程调试 (o3-mini-med)]
    end
```

图 7-5：混合推理模型架构 (Hybrid Reasoning Model Architecture)
...
class CostAwareRouter:
    ...
    COMPLEXITY_THRESHOLDS = {
        "simple": {"model": "gpt-4o", "cost_per_token": 0.005},
...
图 7-6：智能体与推理模型深度整合流程 (Deep Integration of Agents and Reasoning Models)
...
3. **安全对齐**：通过 **审慎对齐 (Deliberative Alignment)** 提升安全性
