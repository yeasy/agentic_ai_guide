...
根据 [Anthropic 官方的逐模型配置表](https://platform.claude.com/docs/en/build-with-claude/thinking-troubleshooting#configurations-each-model-rejects)（核验于 2026-07-25），Claude 的推理能力分为两种模式：
...
- **Adaptive Thinking**（`thinking: {"type": "adaptive"}`）：由模型自主决定是否思考以及思考多深，无需手动设置预算，深度改由 `output_config.effort` 调节。从 Claude 4.6 起的模型都支持——Opus 4.6、Sonnet 4.6、Opus 4.7、Opus 4.8，以及 Sonnet 5、Opus 5、Fable 5、Mythos 5。
- **分界线在 4.7**：Claude 4.7 及之后的模型（Opus 4.7、Opus 4.8、Sonnet 5、Opus 5、Fable 5、Mythos 5）已移除 Extended Thinking，请求中带 `type: "enabled"` 会直接返回 400，只能使用 Adaptive Thinking。迁移方法是去掉 `budget_tokens`、改用 `thinking: {"type": "adaptive"}`，并用 `output_config: {"effort": ...}` 控制推理深度。
