...
- **Extended Thinking**（`thinking: {"type": "enabled"}`）：支持 Claude Haiku 4.5 及更早版本；Claude Opus 4.6 与 Sonnet 4.6 仍可使用 `budget_tokens`，但官方已标记为 deprecated。
- **Adaptive Thinking**（`thinking: {"type": "adaptive"}`）：支持 Claude Opus 4.8、Claude Opus 4.7、Claude Opus 4.6 和 Sonnet 4.6，由模型自主决定推理深度，无需手动设置预算。
