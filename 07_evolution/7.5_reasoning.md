...
截至 **2026 年 4 月**，Anthropic 官方文档对 Claude 4.x 的说明更精确地是：

- **Claude Opus 4.7** 支持 `thinking: {"type": "adaptive"}`，且这是唯一支持的 thinking 模式。
- **Claude Opus 4.6 / Claude Sonnet 4.6** 也支持 adaptive thinking。
- 在 **Opus 4.6 / Sonnet 4.6** 上，手动 `budget_tokens` 模式仍可用，但已被标记为 deprecated。
...
def adaptive_thinking_example(complex_task: str):
    """展示更接近 2026-04 官方文档的 Adaptive Thinking 调用方式"""
...
如果你**不显式设置** `display: "summarized"`，那么在 Opus 4.7 上 thinking 默认是 `omitted`；这时 thinking block 仍可能存在，但其中的 `thinking` 文本为空，真正用于多轮连续性的通常是 `signature` 等协议字段，而不是一段稳定可展示的“完整思维链”。
