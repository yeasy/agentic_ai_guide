...
2024 年以后，推理能力开始从”提示词技巧”逐步变成独立的模型产品能力。以 OpenAI 的 o 系列（自 2024 年起形成推理产品线，后续默认主线进一步演进到 GPT-5）、以及 Anthropic 在 Claude 4.x 中提供的 Adaptive Thinking（旧版 Extended Thinking 已于 2026 年 2 月标记为弃用）为代表，行业趋势变成了：把更多 **测试时计算（test-time compute）** 暴露为可调参数或模式，而不是完全依赖单次前向生成。2026 年 4 月，Anthropic 发布的 Claude Opus 4.7 在高难度软件工程和复杂多步推理上实现了显著提升（SWE-bench Verified 87.6%），同时采用新 tokenizer（同一文本可能多消耗约 0–35% 的 token）。
...
|------|----------|------|------|---------------------|
| **托管式思考模式** | Claude Opus 4.7 / Extended Thinking / Adaptive Thinking 一类能力 | 接入简单，预算可调，与现有工具链结合自然；Opus 4.7 在软件工程和复杂多步推理上有显著提升 | 供应商锁定更强，价格与返回格式依平台变化 | 复杂代码审查、架构设计、关键节点决策 |
