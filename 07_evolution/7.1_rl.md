...
**预训练（Pre-training）** 教会了 LLM "预测下一个词"，但这并不意味着它学会了"如何做一个好的智能体"。要让智能体的行为符合人类的意图（**有用、诚实、无害**，即 3H 原则），我们需要引入强化学习机制。
