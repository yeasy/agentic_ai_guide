...
Cursor 团队在 2026 年初分享了运行数周、协调数百个 Agent 、生成百万行代码的实战经验。这是目前公开的最大规模多智能体协作案例之一。

#### 从扁平到分层的演进

**阶段一：平等协调（失败）**

最初尝试让所有 Agent 地位平等，通过共享文件自协调：

```
Agent 1 → 检查共享状态 → 认领任务 → 更新状态
Agent 2 → 检查共享状态 → 认领任务 → 更新状态
...
问题：锁竞争、遗忘释放锁、Agent 趋向保守
```

结果：20 个 Agent 的实际吞吐量降至 2-3 个的水平。
...
graph TD
    subgraph planning["规划层"]
        planner["Planner"]
        sub_planner1["Sub-Planner 1"]
        sub_planner2["Sub-Planner 2"]
    end
...
- **Planner（大脑）**：探索代码库、创建任务、可递归生成子 Planner
- **Worker（执行）**：专注于单一任务，完成后提交变更
- **Judge（裁判）**：每个周期结束时判断是否继续
...
> **关键发现**：尽管代码库规模庞大，新 Agent 仍能理解并做出有意义的贡献。数百个 Worker 可以并行工作，在同一分支上提交，冲突极少。
...
- 初始设计：有 Integrator 角色负责质量控制和冲突解决
...
1. 提示词比架构更重要——让 Agent 协调良好、避免病态行为、长期保持专注需要大量实验
...
- **任务分析**：理解任务需要哪些能力
- **能力注册**：每个 Agent 声明自己的技能和专长
- **智能匹配**：根据需求选择最优团队配置
- **自适应策略**：规则 + LLM 混合编排

下一节我们将探讨人机协作模式，了解何时应该让人类介入 Agent 的决策过程。
