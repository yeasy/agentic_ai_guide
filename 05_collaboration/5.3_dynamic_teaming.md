...
```mermaid
graph TD
    subgraph Layer1 [Layer 1: 多个专家模型并行处理]
        M1[Llama-4]
        M2[Qwen]
        M3[Mistral]
    end
    
    subgraph Layer2 [Layer 2: 聚合模型综合判断]
        Agg[GPT-5.3<br>聚合器 Aggregator]
    end
...
图 5-5：混合智能体 (MoA) 架构 (Mixture of Agents Architecture)
...
class MixtureOfAgentsOrchestrator:
    def __init__(self):
        self.layer1_models = ["llama-4-70b", "qwen-max", "mistral-large"]
        self.aggregator = "gpt-5.3"
