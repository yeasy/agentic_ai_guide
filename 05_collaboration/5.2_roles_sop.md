...
在多智能体系统中，明确的角色定义和标准化的工作流程是高效协作的基础。本节探讨如何为每个 Agent 设计独特的"人设"，以及如何通过 **标准作业程序（Standard Operating Procedure, SOP）** 来编排复杂的协作流程。
...
| 失败模式 | 发生率 | 描述 |
|---------|--------|------|
| **FM-1.1 违反任务规范** | 11.8% | Agent 未能遵循用户指定的任务要求 |
| **FM-1.2 违反角色规范** | 1.5% | Agent 行为超出其定义的角色边界 |
| **FM-1.3 步骤重复** | 15.7% | Agent 陷入重复执行相同步骤的循环 |
| **FM-1.4 上下文丢失** | 2.8% | 对话历史信息在执行过程中丢失 |
| **FM-1.5 无法识别停止条件** | 12.4% | Agent 无法判断任务何时完成 |

#### 案例：角色规范优化的效果

研究发现，即使使用相同的底层模型（GPT-4o），仅通过改进 Agent 的角色规范设计，ChatDev 框架的成功率就提升了 **+9.4%**。这说明系统设计往往比模型选择更重要。

**优化建议**：

1. **明确终止条件**：在角色定义中显式说明"什么情况下任务完成"，避免 FM-1.5。
2. **防止步骤重复**：设置最大迭代次数，并在 Prompt 中提醒 Agent "不要重复已完成的步骤"。
