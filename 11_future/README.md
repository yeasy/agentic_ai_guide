...
block-beta
    ...
    title("智能体安全的四道防线")
    block:L1
        l1_title("第一道：输入验证与过滤")
        l1_desc("Prompt 注入检测 | 内容过滤 | 结构化输入")
    end
    arrow("↓")
    block:L2
        l2_title("第二道：LLM 自身鲁棒性")
        l2_desc("对抗训练 | 指令层级")
    end
    arrow("↓")
    block:L3
        l3_title("第三道：应用层护栏")
        l3_desc("NeMo Guardrails | Colang 规则")
    end
    arrow("↓")
    block:L4
        l4_title("第四道：架构隔离与最小权限")
        l4_desc("沙箱隔离 | 人机协同 | 输出检测")
    end
```

图 11-0：智能体安全的四道防线 (Four Lines of Defense for Agent Security)

> "With great power comes great responsibility."  
> — 随着智能体能力的增强，对其安全和伦理的责任也在增加。
