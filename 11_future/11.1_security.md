...
2. **指令层级（Instruction Hierarchy）训练**：不仅在提示词中声明优先级，更在模型训练阶段直接植入指令优先级判断能力。
   ...
   仅靠提示词声明是脆弱的——对抗性攻击可以轻易绕过文本层面的约束。OpenAI 的 IH-Challenge[^ih_challenge]（2026）通过**强化学习 + 在线对抗样本生成**直接训练模型区分指令优先级，在 GPT-5-Mini[^gpt5mini] 上将自适应红队攻击鲁棒性从 63.8% 提升至 88.2%。
...
**事件背景**：2026 年 2 月 22 日，一名 Meta 员工报告其使用的 AI 智能体在整理邮箱时误删了约 200 封邮件。虽然影响范围有限（单一用户），但该事件引发了行业对 Agent 安全边界的广泛讨论。以下基于该事件进行扩展的威胁场景分析。
...
[^gpt5mini]: GPT-5-Mini 是 OpenAI 在 2026 年发布的轻量级多模态模型。详见 OpenAI 官方技术文档。
