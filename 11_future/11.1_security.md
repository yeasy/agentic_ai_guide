...
Agent 部署数量激增带来了事件数量的上升，但事件率（单位部署的事件频率）反而下降，说明行业正在学习并改进防御策略：

```
类型                  | 2024年 | 2025年 | 2026年Q1 | 趋势
---------------------|--------|--------|---------|-------
Prompt Injection      | 12     | 45     | 18      | ↓递减
权限提升              | 8      | 22     | 6       | ↓递减
数据泄露              | 5      | 16     | 3       | ↓递减
级联故障              | 3      | 8      | 2       | ↓递减
幻觉导致误操作        | 18     | 52     | 14      | ↓递减
总计严重事故          | 46     | 143    | 43      | ↑增长后稳定
```
...
等级    | 特征                | 影响范围 | 恢复时间 | 案例数 |
--------|-------------------|---------|--------|------|
Critical| 数据丢失或核心系统宕机| 企业级  | >1天  | 8     |
High    | 财务损失或隐私泄露   | 部门级  | 4-24h | 18    |
Medium  | 功能故障或性能下降   | 功能级  | 1-4h  | 12    |
Low     | 用户体验问题        | 用户级  | <1h   | 5     |
```

#### 案例一：Meta 邮件误删事件

**事件背景**：Meta 内部知识管理 Agent (IKMA) 在 2026 年 2 月 14 日误删 100+ 用户的邮件记录，导致 6 小时业务中断。
...
**事件背景**：Stripe 部署的支付自动化 Agent 通过精心设计的提示注入，被攻击者引导使用管理员 API 密钥，导致 10 万个测试账户的支付记录被篡改，估计损失 $50K。
...
#### 案例三：ChatGPT插件幻觉导致误交易

**事件背景**：用户通过 ChatGPT 插件购买股票，插件基于不存在的股票代码生成交易订单。虽然订单最终被 broker 拒绝，但反映了 Agent 系统的幻觉问题。
