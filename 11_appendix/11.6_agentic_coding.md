...
**Rules 最佳实践**：
- 聚焦必需项：构建命令、核心模式、典型示例的指针
- 引用文件而非复制内容，避免 Rules 与代码脱节
- 团队共享：检入 Git，让全员受益

**何时添加 Rule**：只有当你注意到 Agent 反复犯同一错误时才添加。避免过度优化。
...
- **PostToolUse + Write**：每次写入文件后自动运行 linter
- **PreToolUse + Bash**：拦截 `rm -rf` 或 force push
- **SessionStart**：加载当前 Sprint 的任务上下文
