...
2025 年，随着 Agent 系统复杂度的提升，业界逐渐意识到：仅仅优化 Prompt 已经不够了。一个全新的工程学科正在兴起——**Context Engineering（上下文工程）**。

> [!IMPORTANT]
> **行业信号**：LangChain CEO Harrison Chase 在 2025 年 Interrupt 大会上指出："Context Engineering 是构建可靠 Agent 的核心技能。我们需要的不仅仅是 Prompt Engineer，而是 Agent Engineer——一个融合提示、工程、产品设计和机器学习的新角色。"

### 2.6.1 什么是 Context Engineering

**Context Engineering** 是系统性地管理、优化和编排输入给 AI 模型的所有信息的工程学科。
...
| 复杂度 | 相对简单 | 系统工程级别 |
| 动态性 | 通常静态 | 高度动态、上下文感知 |
| 角色 | Prompt Engineer | Agent Engineer |
...
LangChain 定义了 Context Engineering 的四大策略，对应不同的上下文管理场景：
...
**隔离原则**：
- 最小知识原则：Agent 只获取完成任务所需的信息
- 减少干扰：无关上下文会降低推理精度（Lost in the Middle）
- 安全边界：敏感信息隔离

### 2.6.3 Agent Engineer：新角色的诞生

Harrison Chase 提出，构建可靠 Agent 需要一种新的复合型人才——**Agent Engineer**：
...
| Agent Engineering | 构建自主系统 | 最高 |

**核心原则**：
1. **精确控制**：Agent 的输出质量取决于输入上下文的质量
2. **动态适应**：上下文应根据任务、用户、环境动态调整
3. **资源意识**：Token 是昂贵的，必须高效利用
4. **系统思维**：将上下文管理视为独立的工程子系统

### 2.6.6 核心约束：名义上下文 vs 有效上下文
...
*   **经验法则**：Coding Agent 通常只能有效利用 **10-15%** 的上下文窗口。超过 20% 后，性能（推理准确性、指令遵循）开始急剧恶化。

#### 2. The Dumb Zone (智能陷没区)
研究发现，上下文窗口的 **中间 40-60% 区域** 存在“Dumb Zone”。
*   **首尾偏好**：Model 倾向于关注开头（System Prompt）和结尾（最新对话）。
*   **Lost in the Middle**：关键信息如果被淹没在中间的日志海中，很容易被忽略。
...
> [!WARNING]
> **上下文腐烂（Context Rot）**：当 Agent 调用工具次数增多，消息列表不断膨胀，推理性能会出现断崖式下跌——推理变慢、质量下降、甚至开始无意义地重复。
...
**Manus 观察**：典型的 Agent 任务大约需要调用 50 次工具。Anthropic 报告生产环境中的 Agent 可能进行数百轮对话。
...
核心思路：**不要把所有东西都硬塞进 Agent 的短期记忆里，而是卸载到外部存储，需要时再检索回来**。
...
### 2.6.9 Cursor 动态上下文发现

Cursor 的核心理念：**少即是多**——在开始时提供给模型的细节越少，效果反而越好。

#### 万物皆文件

Cursor 将所有可能的上下文都转化为文件，让 Agent 按需检索：
...
### 2.6.10 分层行动空间：解决工具过载

当 Agent 配备的工具越来越多，会出现**上下文混淆**——模型可能调用错误的工具，甚至幻觉出不存在的工具。
...
| 工具管理 | 文件化定义，Agent 自行查阅 | 分层行动空间，L1 固定 |
| 迭代方向 | 简化架构 | 移除不必要的技巧 |
...
>
> **这是 Agent 工程师最重要的认知升级：把精力从"追逐更大的模型"转向"设计更优的系统"。**
...
> - Manus: [如何构建 AI Agent 的上下文工程](https://mp.weixin.qq.com/s/xxx)
