...
| 要素 | 说明 | 示例 |
|------|------|------|
| **思考（Thought）** | 思考过程，分析当前状态和下一步计划 | "我需要先了解用户的具体需求..." |
| **行动（Action）** | 执行的具体操作，通常是工具调用 | `search(query="...")` |
| **观察（Observation）** | 行动的结果，来自环境的反馈 | "搜索返回了 5 条相关结果..." |
...
ReAct 架构是现代智能体系统的基础范式，它实现了：

- 推理与行动的统一
- 动态环境交互
- 可解释的决策过程

下一节我们将探讨 Reflexion，学习如何让智能体从失败中学习和自我改进。
