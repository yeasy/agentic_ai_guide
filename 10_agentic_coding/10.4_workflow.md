...
### 10.4.1 智能体开发闭环 (The Agentic Development Cycle)
...
```mermaid
graph TD
    subgraph P [1. 规划 Prepare]
        P1[需求理解] --> P2[上下文组装]
    end

    subgraph D [2. 分解 Decompose]
        D1[任务拆解] --> D2[制定计划]
    end

    subgraph E [3. 执行 Execute]
        E1[智能体编码] --> E2[工具调用]
    end

    subgraph R [4. 审查 Review]
        R1[人工验收] --> R2[自动测试]
    end
...
图 10-14：P-D-E-R 智能体开发循环 (P-D-E-R Agentic Development Cycle)
...
图 10-15：智能体操作禁止清单 (Agent Operation Forbidden List)
...
图 10-16：用户认证模块任务分解思维导图 (Task Decomposition Mindmap for User Auth Module)
...
图 10-17：规划优先的开发流程 (Plan-First Development Process)
...
图 10-18：TDD 驱动的智能体工作流 (TDD-Driven Agent Architecture)
...
图 10-19：认证系统序列图示例 (Auth System Sequence Diagram Example)
