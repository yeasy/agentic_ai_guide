...
Cline (原 Claude Dev) 是一个优秀的开源 VS Code 自主智能体扩展。它既支持连接云端模型，也支持通过 Ollama、LM Studio 等运行时接入本地模型，是“本地化与自托管”场景的代表性方案。如果对数据隐私有严格要求，或希望更强的可控性与成本边界，可以选择 Cline 配合本地模型运行时。
...
Claude Code 是一款强大的 CLI 工具，擅长与终端工作流融合：批量修改、运行脚本、分析 Git 历史、生成变更摘要等。除终端外，它还提供面向 VS Code / Cursor / Windsurf 及 JetBrains 家族的 IDE 集成。
...
Claude Code 的扩展面主要由 `CLAUDE.md` / memory、`.claude/settings.json`、Hooks、Subagents 与 MCP 组成：你可以用 `CLAUDE.md` 管理项目级记忆，用 Hooks 在工具调用前后插入校验逻辑，用 Subagents 做角色拆分，并通过 MCP 连接外部工具与数据源。
