...
Claude Code 支持 `effort` 参数控制推理深度（`low` / `medium` / `high` / `xhigh` / `max`）。其中 `xhigh` 在 Opus 4.8 与 Opus 4.7 上可用（且是 Opus 4.7 的默认档位；Opus 4.8、Opus 4.6 与 Sonnet 4.6 默认为 `high`），`max` 提供最深度推理但 token 消耗最高。`xhigh` 适合复杂重构、多文件迁移和架构级任务——它会启动多轮规划，覆盖边界条件与跨模块依赖，而 `high` 通常只生成主路径实现。推荐策略是日常使用 `high`，深度推理场景切换到 `xhigh`，仅在评测显示 `xhigh` 仍有提升空间时使用 `max`。
