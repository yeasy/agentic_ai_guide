...
graph TD
    ...
    subgraph Underwater ["🧊 冰山之下 (隐性状态)"]
        ...
        subgraph ProcessLayer [" "]
            ...
            subgraph CoT ["🧠 思维链 (Chain of Thought)"]
                direction TB
                Search[搜索]:::submerged --> Read[读取]:::submerged --> Failed[修正]:::submerged
            end

            subgraph Ctx ["👁️ 动态感知 (Context)"]
                direction TB
...
根据 Cursor 团队的定义，一个完整的智能体驾驭系统由三层核心组件构成：消息、工具和指令。
...
graph TD
    ...
    class Edit,Search,Term,Browser,MCP,Custom tool;
...
> [!TIP]
> 高效的 Agentic Coding 需要学会通过 Rules 和 Skills 来调整驾驭机制，以适配不同模型的特点。
