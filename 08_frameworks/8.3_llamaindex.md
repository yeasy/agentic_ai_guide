...
[LlamaIndex](https://www.llamaindex.ai/) 是专注于数据连接和检索增强生成（RAG）的框架。虽然它以 RAG 能力闻名，但 LlamaIndex 也提供了强大的 Agent 构建能力，特别适合需要处理大量非结构化数据的智能体应用。

### 8.3.1 LlamaIndex 的定位

### 8.3.2 核心哲学

LlamaIndex 的设计理念是 **"Data-Centric AI"**——以数据为中心：

```
传统 Agent:  用户 → LLM → 工具 → 结果
LlamaIndex:  用户 → LLM → 数据索引 → 知识检索 → 结果
```

### 8.3.3 与 LangChain 的区别

| 维度 | LangChain/LangGraph | LlamaIndex |
|------|---------------------|------------|
| 核心关注 | 工作流编排 | 数据索引与检索 |
| 强项 | 复杂控制流 | RAG 与知识管理 |
| Agent 模式 | 通用 Agent | 数据 Agent |
| 适用场景 | 多步骤任务 | 知识密集型任务 |
...
| 类型 | 用途 | 适用场景 |
|------|------|----------|
| VectorStoreIndex | 语义搜索 | 通用问答 |
| SummaryIndex | 全文摘要 | 文档总结 |
| TreeIndex | 层级检索 | 结构化文档 |
| KnowledgeGraphIndex | 知识图谱 | 实体关系查询 |

### 8.3.6 Query Engine（查询引擎）
...
### 8.3.11 SubQuestion Query Engine
...
- **灵活的索引**：多种索引类型满足不同需求
- **RAG 优化**：内置多种检索和后处理策略
- **Query Engine as Tool**：将知识库封装为 Agent 工具
...
下一节我们将探讨多智能体框架 AutoGen 和 CrewAI。
