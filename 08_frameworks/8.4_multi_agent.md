...
> **生态口径（截至 2026-04）**：Microsoft AutoGen 官方仓库已明确进入 maintenance mode，并建议新项目转向 **Microsoft Agent Framework**；AG2 官方则把自己定义为 **AG2 (formerly AutoGen)**，也就是从 AutoGen 演化出的社区主线。与此同时，CrewAI 的稳定发布线已经进入 **1.14.x**。
...
```python
# CrewAI 示例（截至 2026-04 属于 1.14.x 系列 API）：用“角色 + 任务流”构建市场调研团队
...
| 维度 | 对话编排范式 | 角色任务编排范式 |
| :--- | :--- | :--- |
| **代表框架** | AG2（社区延续，AutoGen 风格） / Microsoft Agent Framework（Microsoft 当前主线） | CrewAI（截至 2026-04 为 1.14.x 稳定线） |
...
- 一类框架更偏“角色流程”（研究→写作→审阅→交付），典型如 CrewAI（截至 2026-04 的稳定线已进入 1.14.x）。
