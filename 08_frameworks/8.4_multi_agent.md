...
单体 Agent（Single Agent）受限于上下文窗口和角色的单一性，难以处理极其复杂的任务。这就好比一个人再厉害，也无法独自完成一部电影的拍摄。

**多智能体系统 (Multi-Agent System, MAS)** 通过模拟人类团队的协作模式，让多个专注于不同领域的 Agent 共同解决问题。本节将通过实战案例，深入解析两大主流 MAS 框架：AutoGen 和 CrewAI。
...
# 1. 定义角色
# 详细的 Backstory 能显著提升 Agent 的"代入感"
researcher = Agent(
    role='高级市场研究员',
...
- 通过让 Researcher 专注搜索，Writer 专注写作，每个 Agent 的 System Prompt 都可以更短、更聚焦，从而降低幻觉，提升质量。
- **AutoGen** 是"代码工厂"，适合构建能干活的软件机器人。
- **CrewAI** 是"创意工作室"，适合构建能思考的内容团队。

下一节我们将探讨如何将这些 Agent 集成到现有的企业系统中。
