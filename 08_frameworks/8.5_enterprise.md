...
> **重要变化（2026）**：Microsoft 于 2025 年 10 月推出 **Microsoft Agent Framework**，正式合并 Semantic Kernel 与 AutoGen 为统一平台。Microsoft Agent Framework 1.0 GA 于 2026 年 4 月 3 日发布。Semantic Kernel v1.x 将继续维护至少 1 年，但新特性优先在 Microsoft Agent Framework 上开发。本节以 Semantic Kernel 1.41.2（2026 年 4 月）为例，同时介绍 Microsoft Agent Framework 的过渡路径。
...
- **Semantic Kernel 1.x 时代**（当前维护）：成熟稳定，适合生产环境。
...
# Semantic Kernel 1.41.2 示例：初始化 Kernel、注册服务与插件，并启用自动函数调用
...
async def main():
    ...
    # 2. 配置 AI 服务（Semantic Kernel 1.41+ 中，API 已从 add_model_service 改为 add_service）
...
Microsoft Agent Framework（1.0 GA）在 Semantic Kernel 基础上增强了：
