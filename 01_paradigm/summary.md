...
2.  **理论基石：理性智能体**
    ...
    *   **PEAS 框架**：设计智能体时必须明确其性能目标 (P)、环境 (E)、执行器 (A) 和传感器 (S)。
...
3.  **解剖学结构：四大核心组件**
    ...
    *   **行动 (Action/Tools)**：通过**函数调用（Function Calling）** 使用外部工具（搜索、代码执行、API），是智能体改变世界的手。
    *   **记忆 (Memory)**：克服**上下文窗口**限制，结合短期记忆（上下文）与长期记忆（向量数据库/检索增强生成 RAG），实现经验的积累。
    
4.  **认知层级：从助手到群体**
    *   将智能体能力划分为 5 个层级：**L1 辅助型** (对话)、**L2 执行型** (工具)、**L3 自主型** (学习)、**L4 进化型** (自我编码)、**L5 群体型** (协作)。这一模型指导我们根据任务复杂度选择合适的架构。

### 下一步
理解了“智能体是什么”和“由什么组成”之后，下一章我们将深入微观层面，详细讲解智能体如何像人类一样进行复杂的**规划**、构建**记忆**以及熟练地**使用工具**。
...
*   [**ReAct: Synergizing Reasoning and Acting in Language Models**](https://arxiv.org/abs/2210.03629): 奠定了现代智能体 "推理-再行动" 的基础模式。