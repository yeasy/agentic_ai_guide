## 1.1 从 LLM 到 Agent：范式转移

### 1.1.1 引言：被动与主动的界线

在人工智能的发展长河中，我们正经历着从“生成式 AI (Generative AI)”向“代理式 AI (Agentic AI)”的深刻转型。如果说 2023 年是 ChatGPT 惊艳世界的“对话元年”，那么 2025 年及以后将属于能够独立完成任务的 **AutoGPT** 和 **Agents**。

传统的大语言模型（LLM）像是一个博学但瘫痪的思想家。它阅尽了互联网上的所有文本，拥有关于莎士比亚、量子物理和 Python 编程的无穷知识，但它被困在参数的牢笼里。它无法点击鼠标，无法通过 API 发送邮件，更无法感知时间的流逝。它唯一的输出就是文本。

**Agentic AI** 的出现打破了这一僵局。通过赋予 LLM “手”（工具）、“眼”（感知）和“记忆”，我们将这个思想家变成了一个能够与物理和数字世界互动的实干家。这种范式转移（Paradigm Shift）不仅是技术的升级，更是人机交互模式的根本性变革——我们不再是命令机器“生成一段文字”，而是授权机器“帮我达成一个目标”。
...
### 1.1.2 核心差异：从 Next Token Prediction 到 Goal Achievement

要理解 Agent，首先要剖析它与 LLM 的本质区别。

#### 目标导向 (Goal-Oriented) vs. 概率预测
LLM 的训练目标是极其单一的：根据上文预测下一个 Token 的概率分布。当你问它“这种药怎么吃？”时，它并不是为了治好你的病而回答，仅仅是因为在训练数据中，这样的问题通常伴随着说明书式的回答。
相反，Agent 是**目标导向**的。它内置了一个核心指令（System Prompt）："你是一个能够解决问题的助手"。当面对任务时，它不急于输出 Token，而是先思考“为了达成这个目标，我需要获取什么信息？需要执行什么步骤？”。
...
Agent 架构实际上是在 LLM 这个强大的“系统 1”外层，用工程手段（Prompt Engineering, RAG, Tool Use）包裹了一层“系统 2”的逻辑外壳。

#### 为什么现在爆发？

Agent 的概念在几十年前的 AI 教科书（如 AIMA）中就是核心，为何直到 2023/2024 年才爆发？这源于三大要素的“涌现”：

1.  **推理能力的临界点**：
    早期的 AI（如 BERT, GPT-2）缺乏逻辑推理能力，无法进行有效的任务拆解。直到 GPT-3.5/GPT-4 的出现，模型展现出了强大的 **In-context Learning** 和逻辑推演能力，使得“让模型自己写计划”成为可能。

2.  **标准化的交互协议**：
    **Function Calling (Tool Use)** 的出现是决定性的。OpenAI 将“调用外部工具”这一行为标准化为 JSON 格式的结构化数据。这让模型与现实世界的软件系统（Calculator, Search API, Database）打通了最后一公里。
