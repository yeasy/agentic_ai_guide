...
我们将 User (用户)、Model (模型)、Tool (工具) 视为三个交互节点。整个 Agent 的运行过程，就是 Context Window（上下文窗口）随着时间推移不断“吞噬”新状态的过程：
...
*   **文件系统即记忆**：对于 Coding Agent，文件系统的当前状态就是最大的“外部记忆”。每一次 `write_file` 都是对外部记忆的写入。
...
*   **Chain-of-Thought (CoT) 的本质**：为什么让模型“一步步思考”能提升效果？因为你实际上是给了它更多的**计算步骤**（即更多的 Token 生成机会）来辅助推理。CoT 只是通过生成中间步骤，展开了推理过程。
