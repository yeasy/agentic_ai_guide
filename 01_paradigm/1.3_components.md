...
graph TD
    ...
    subgraph TCB["安全边界 (Trusted Computing Base)"]
        Sandbox["🛡️ 策略网关与沙箱<br/>(拦截非法调用/脱敏)"]
    end

    UserNode -.-> Perception
    SystemNode --> Perception
    Action --> Sandbox
    Sandbox --> SystemNode
    
    class UserNode user;
    class SystemNode env;
    class Perception,Brain agent;
...
> 这是生产级智能体区别于“玩具 Demo”的分水岭。大脑虽然聪明，但也是最容易被攻击和产生幻觉的部分（非完全可信）。因此，在将行动下发至真实物理/数字系统前，必须经过一层包含硬编码策略引擎、权限校验和沙箱隔离的 TCB 拦截层。
