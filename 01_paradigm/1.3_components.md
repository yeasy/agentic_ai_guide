...
#### 多模态感知
现代 Agent 正在长出眼睛和耳朵。
*   **视觉 (Vision)**：利用 GPT-5 或 Claude 4 的视觉能力，Agent 可以"看"懂网页的 DOM 结构，识别屏幕上的按钮坐标，甚至通过看监控摄像头来判断是否有行人。
...
*   它就像计算机的 **硬盘**。
*   **检索增强生成（RAG）**：当记忆太多放不下 Context 时，Agent 会先在向量库中检索与当前问题最相关的几条片段，只把这几条“回忆”加载到短期记忆中。
