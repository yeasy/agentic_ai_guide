...
```mermaid
graph TD
    UserQuery["用户查询 (User Query)"] --> Embed["1. 嵌入 (Embed): 将查询转换为向量"]
    Embed --> Retrieve["2. 检索 (Retrieve): 从向量库检索相关文档"]
    Retrieve --> Augment["3. 增强与生成 (Augment & Generate)"]
    
    subgraph process ["生成过程"]
        Augment --> Prompt["提示词 (Prompt) = 检索到的文档 + 用户查询"]
        Prompt --> Generate["LLM 根据增强后的提示词生成回答"]
    end
    
    Generate --> Answer["最终回答 (Visual Answer)"]
```
```
...
| Faithfulness | 回答与资料的一致性/忠实度 | LLM 评估或人工标注 |
| Answer Relevancy | 回答与问题的相关性 | LLM 评估 |
...
1. **检索质量**：好的切分 + 好的 Embedding + 重排序
2. **生成质量**：精心设计的 Prompt + 诚实的不确定性表达
3. **持续评估**：建立评估流水线，持续优化

下一节我们将探讨上下文窗口管理，学习如何在有限的窗口中最大化信息利用。

---

**下一节**: [上下文窗口管理与压缩策略](3.4_context_management.md)