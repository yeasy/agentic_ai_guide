...
本节关注 **短期记忆（Short-term Memory）** 的具体实现算法与数据结构。关于更高层面的工程策略（如上下文工程、动态行动空间），请参考 [**3.6 上下文工程**](3.6_context_engineering.md)。

短期记忆通常对应于大模型的 **上下文窗口**。虽然 2025 年的模型支持 1M+ 的上下文，但高效的窗口管理算法仍然是降低延迟和成本的关键。
...
上下文管理可以从两个维度入手：**选择策略**（保留什么）和 **压缩策略**（如何压缩）。
