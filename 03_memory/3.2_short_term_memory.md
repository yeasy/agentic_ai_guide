...
```mermaid
graph TD
    A["上下文窗口总预算<br/>Max Tokens: 4k-128k"] --> B["系统提示词<br/>500-2k<br/>身份、规则、工具定义"]
    A --> C["检索的知识/记忆<br/>1k-5k<br/>外部 RAG 内容"]
    A --> D["对话历史<br/>2k-10k<br/>之前的对话轮次"]
    A --> E["当前用户输入<br/>100-1k<br/>本轮请求"]
    A --> F["预留输出空间<br/>1k-4k<br/>模型生成回复"]

    style A fill:#e6f7ff,stroke:#1890ff,stroke-width:2px
    style B fill:#f6ffed,stroke:#52c41a,stroke-width:1px
    style C fill:#fff0f6,stroke:#eb2f96,stroke-width:1px
    style D fill:#fff7e6,stroke:#faad14,stroke-width:1px
    style E fill:#f9f0ff,stroke:#722ed1,stroke-width:1px
    style F fill:#f0f5ff,stroke:#177ddc,stroke-width:1px
```
