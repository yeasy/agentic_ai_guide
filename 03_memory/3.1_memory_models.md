...
大语言模型的本质是无状态的函数。虽然最新的 Gemini 2.0 支持 2M+ 的**上下文窗口（Context Window）**，但在实际工程中，完全依赖 Context Window 面临三个核心挑战：
...
结合**知识图谱（Knowledge Graph）** 和**向量数据库（Vector DB）** 的 **GraphRAG** 技术正在兴起：
