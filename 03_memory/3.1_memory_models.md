...
在设计智能体（Agent）之前，我们需要先回答一个根本性问题：**智能体的"记忆"到底是什么？**

如果不理解记忆的本质，我们很容易陷入"把所有东西都塞进把向量数据库"的误区。本节将从认知科学和计算机工程的双重视角，重新解构智能体的记忆系统。
...
```mermaid
graph TD
    Input["环境输入<br>(Environment Input)"] --> Sensory["感官记忆<br>(Sensory Memory)"]
    Sensory -->|"注意 (Attention)"| Working["工作记忆<br>(Working Memory)"]
    Working <-->|"检索/存储"| LongTerm["长期记忆<br>(Long-term Memory)"]
    
    subgraph LongTermSystem ["长期记忆系统"]
        Episodic["情景记忆 (Episodic)"]
        Semantic["语义记忆 (Semantic)"]
        Procedural["程序/技能记忆 (Procedural)"]
        Reflective["反思记忆 (Reflective)"]
    end
    
    Working --> Output["动作/响应<br>(Action/Response)"]
    LongTerm --> Working

    classDef distinct fill:#f9f,stroke:#333,stroke-width:2px;
    class Working distinct;
```
...
| :--- | :--- | :--- | :--- | :--- |
| **情景记忆**<br>(Episodic) | 个人经历的序列纪录 | "我昨天吃了寿司" | 对话日志、交互历史 | 向量数据库 (Vector DB) |
| **语义记忆**<br>(Semantic) | 事实与世界知识 | "寿司是日本食物" | 领域文档、百科知识 | 知识图谱 (Knowledge Graph) / RAG |
| **程序记忆**<br>(Procedural) | 执行任务的技能 | "如何骑自行车" | 工具 (Tools)、代码解释器 | 函数定义 (Function Schemas) |
| **反思记忆**<br>(Reflective) | 元认知与经验教训 | "我不该空腹吃生鲜" | 优化后的 Prompt、SOP 改进 | 提示词库 / 元数据存储 |

> **补充：反思记忆 (Reflective Memory)**
...
Anthropic 在 2025 年推出的 Claude Memory 是一个典型的 **"用户感知型" (User-aware) 反思记忆** 系统。它不只是简单地存储对话日志，而是提取用户的 **偏好 (Preferences)** 和 **背景 (Context)**。
...
**下一节**: [3.2 短期记忆算法](3.2_short_term_memory.md) 将深入探讨如何高效管理昂贵的 Context Window。
