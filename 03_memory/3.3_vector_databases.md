...
向量数据库是最常见的存储智能体长期记忆的方式。它们专门优化了 **向量 (Embeddings)** 的存储和相似性搜索，是构建 RAG 系统和记忆系统的关键组件。
...
图 3-3：向量数据库检索流程 (Vector Database Retrieval Process)
...
图 3-4：向量数据库选型决策树 (Vector Database Selection Decision Tree)
...
**下一节**: [3.4 RAG 进阶](3.4_rag_advanced.md)
