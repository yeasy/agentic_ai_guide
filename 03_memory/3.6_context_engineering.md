...
graph TD
    ...
    class PE1,PE2,PE3 agent;
    class CE1,CE2,CE3,CE4,CE5,CE6 tool;
```
...
> [!TIP]
> **氧气瓶理论**：上下文窗口就像潜水员的氧气瓶。给你一个更大的氧气瓶（更长上下文），不代表你能一直潜水直到用完。在深度（复杂度）增加时，耗氧量剧增，你必须在“氧气耗尽”前浮出水面（重启会话）。
...
这是 **RAG**的本质。即便拥有很长的上下文窗口，将整本技术手册塞入 提示词通常也是低效且昂贵的。“筛选上下文”就像是为模型佩戴一副“滤镜”，过滤掉噪音，只保留信号。**关键操作**：
...
class ContextPack:
    ...
    def __init__(self, trace_id: str = None):
        ...
        self.tools: List[Dict] = []        # 当前可用工具的 Schema
        self.memory: List[Dict] = []       # 长/短期记忆、压缩历史及偏好
        self.evidence: List[Dict] = []     # 检索到的强事实片段（需含 source_id）
        
        # 审计与计费元数据区
        self.audit: Dict[str, Any] = {
            "timestamp": datetime.now().isoformat(),
            "token_budget": {"used": 0, "remaining": 8000},
            "routing_tags": []
        }

    def to_llm_payload(self) -> List[Dict]:
        """将 ContextPack 渲染为传递给 LLM 的 messages 数组格式"""
        # 此处省略具体的序列化逻辑...
        pass

class ContextManager:
    """基于 ContextPack 的统一上下文管理器"""
    ...
    def prepare_context(
        ...
        """为智能体调用组装标准 ContextPack"""
        
        pack = ContextPack(trace_id=parent_trace_id)
        
        # 1. 加载持久化的用户上下文（存入 memory）
        user_prefs = self.writer.memory.read(f"user:{user_id}:preferences")
        if user_prefs:
            pack.memory.append({"role": "system", "content": f"[User Pref] {user_prefs}"})
        
        # 2. 筛选相关知识（存入 evidence）
        pack.evidence = self.selector.select_relevant_context(
            query, max_tokens=4000
        )
        
        # 3. 压缩对话历史（存入 memory）
        pack.memory.extend(self.compressor.compress_conversation_history(
            conversation_history, max_tokens=2000
        ))
        
        # 4. 过滤相关工具
        pack.tools = self._filter_relevant_tools(query, available_tools)
        
        # 5. 更新审计预算
        pack.audit["token_budget"]["used"] = self._count_total_tokens(pack)
        pack.audit["token_budget"]["remaining"] -= pack.audit["token_budget"]["used"]
...
> [!WARNING]
> **上下文腐烂**：当智能体调用工具次数增多，消息列表不断膨胀，推理性能会出现断崖式下跌——推理变慢、质量下降、甚至开始无意义地重复。
...
graph TD
    ...
    class T1,T2 agent;
    class T50,Hist50,Lost error;
    class Hist1,Hist2 agent;
```
...
* **上下文预算与裁剪**：能把“成本、延迟、可用信息”三者权衡落到可执行策略。
* **信息检索与证据链**：能设计检索、重排序与引用格式，让答案可追溯、可审计。
* **工具与接口设计**：能把复杂能力封装为稳定的原子工具，并通过 Schema 降低参数幻觉。
* **评估与回归**：能把需求转化为验收标准，维护回归样例集并量化改动收益。
* **安全与隐私**：能处理提示词注入、数据脱敏、权限分级与保留期治理。
* **可观测与故障恢复**：能用轨迹日志定位失败模式，并设计重试、回退与熔断机制。
