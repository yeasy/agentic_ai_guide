...
在 Anthropic 的生产系统中，这三个角色通常由以下方式实现：

* **Planner**：Claude Opus 4.7（更强的规划能力）或专用的规划提示，生成结构化的步骤清单
* **Generator**：可以是任何模型（Claude Sonnet 4.6、GPT-5.4、开源模型），严格遵循计划
...
行业验证：LangChain 的 Deep Agents 通过实现 PGE 框架，在 Terminal Bench 2.0 上的成绩从 52.8% 跃升至 66.5%，充分说明了分离的威力。这一改进本质上是 Harness 级别的优化——不是更强的模型，而是更聪明的任务分解和验收机制。
