...
Agent 系统的运营成本主要来自 LLM API 调用。一个设计不当的 Agent 可能每个任务消耗数万 Token，导致成本失控。本节探讨如何在保证质量的前提下优化性能和控制成本。
...
class CascadeAgent:
    async def process(self, query: str) -> str:
        # 第一层：尝试便宜模型
        response = await self.call_model("gpt-3.5-turbo", query)
