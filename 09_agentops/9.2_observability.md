...
在传统的软件开发中，通常使用 **调用栈 (Stack Trace)** 报错堆栈。但在智能体开发中，"Bug" 往往不是代码崩溃（Crash），而是智能体 "一本正经地胡说八道" 或者 "陷入了无意义的死循环"。

当智能体出错时，你不能只看最后一行输出。你需要像黑匣子一样，记录下它每一次思考、每一次工具调用和每一次状态更新。这就是智能体的 **可观测性 (Observability)**。

### 9.2.1 为什么智能体难以调试

1.  **非确定性 (Nondeterminism)**：同样的提示词，上午跑是通的，下午跑就挂了。这使得复现 Bug 变得极其困难。
2.  **黑盒推理 (Black-box Reasoning)**：LLM 为什么决定调用 Search 而不是 Calculator？这种决策过程不可见。
3.  **多步级联 (Cascading Failures)**：第 5 步的错误可能是第 1 步的一个微小偏差导致的蝴蝶效应。
4.  **成本隐患 (Cost)**：一个死循环的智能体可能在一夜之间烧掉几千美元。

### 9.2.2 核心指标：Trace, Span 与 Run

借鉴 **分布式追踪 (Distributed Tracing)** 的概念，可以构建智能体的监控数据模型：

*   **链路 (Trace)**：代表用户的一次请求（Request）处理全过程。
*   **跨度 (Span / Run)**：链路中的执行片段（Execution Step）。它既可以指代一次简单的工具调用，也可以包含多个子步骤（Child Spans），共同构成树状结构。
...
在实际的链路追踪平台（如 LangSmith）中，你不仅能看到这个时序图，还能点击每个 Span 查看具体的 **输入 (Input)**, **输出 (Output)**, **延迟 (Latency)** 和 **成本 (Cost)**。
...
*   **方法**: 运行一组 **黄金标准问题 (Golden Questions)**，计算准确率（Accuracy）或相似度（Similarity）。
*   **工具**: Pytest, LangSmith Dataset。

#### 2. 在线评估 (Online Evaluation & Monitor Agents)
这是生产环境的关键。既然无法人工检查每一条日志，可以部署 **监控智能体 (Monitor Agents)** 来实时打分。

*   **原理**: 
    *   主智能体 (Main Agent) 处理用户请求。
    *   监控智能体 (Evaluator Agent) 异步读取主智能体的 Input/Output，进行审计。
*   **检测项**:
...
当多智能体系统（MAS）出现问题时，如何系统性地定位根因？研究论文《[Why Do Multi-Agent LLM Systems Fail?](https://arxiv.org/abs/2503.13657)》提出了 **MAST (Multi-Agent System Failure Taxonomy)** 分类法。

MAST 将 14 种失败模式归纳为 3 大类别。当你面对一个错误的链路时，可以按此表检索：

| 类别 | 失败模式 (Failure Mode) | 典型表现 | 修复策略 |
| :--- | :--- | :--- | :--- |
...
1.  **Capture (捕获)**:
    *   全量记录线上链路。
2.  **Curate (筛选)**:
    *   利用 **在线评估** (Monitor Agents) 自动筛选出得分低的 `Bad Cases`。
    *   这是最高价值的数据，因为它们代表了当前系统的短板。
3.  **Correct (修正)**:
    *   人工介入，修正这些 Bad Cases 的预期输出（Expected Output）。
    *   将它们加入到 **离线评估数据集 (Golden Dataset)** 中。
4.  **Improve (改进)**:
