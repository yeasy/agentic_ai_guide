...
业界已经涌现出一批优秀的 AgentOps 平台，帮助开发者追踪、调试和评估智能体。本节将介绍 LangSmith、Arize Phoenix 和 Weights & Biases 这三款代表性工具。
...
### 9.2.9 数据飞轮：从 Trace 到 Dataset
 
 可观测性的终极价值，在于打通 **"线上监控 (Ch9)"** 与 **"离线评估 (Ch7)"** 的闭环，构建 **数据飞轮 (Data Flywheel)**。
 
 ```mermaid
 flowchart LR
     subgraph Online["线上环境 (Production)"]
         Log[运行日志] --> Trace[LangSmith Trace]
         Trace --> Filter{筛选器}
         Filter -->|Bad Case| Human[人工复核]
         Filter -->|Good Case| AutoGen[合成数据]
     end
 
     subgraph Offline["开发环境 (Dev)"]
         Human -->|标注修正| Golden[黄金数据集]
         Golden --> Eval[自动化评估 (Ch7)]
         Eval -->|Pass| Deploy[部署上线]
     end
 
     Deploy --> Log
 ```

图 9-1：基于可观测性的智能体数据飞轮
 
 **实施步骤**：
 
 1. **捕获 (Capture)**：利用 Tracing 工具全量记录线上对话。
 2. **筛选 (Filter)**：
    *   **显式反馈**：用户点了 "Thumbs Down" 的 Trace。
    *   **隐式反馈**：用户多次重试或修改 Query 的 Trace。
    *   **自动检测**：Eval 智能体判定为 "Hallucination" 的 Trace。
 3. **固化 (Curate)**：将这些失败的 Trace 转化为测试用例 (Test Case)。
    *   Input: 用户的原始 Query
    *   Expected Output: 人工修正后的正确答案
 4. **回归 (Regress)**：将新的一批 Test Case 加入到 CI/CD 的评估集 (Dataset) 中。
 
 > **只有当昨天的生产事故变成了今天的自动化测试题，你的智能体系统才算真正拥有了"记忆"。**
