...
*   **优势**：极低的延迟，易于调试，每个节点可以使用不同规格的模型（例如大纲用 GPT-5.3，扩写用 GPT-5.3-mini）。
...
graph LR
    ...
    Router -->|复杂问题| HandlerC[高级 Agent]
```

*   **适用场景**：
    *   **客服分流**：根据用户意图（售后/投诉/咨询）路由到不同处理模块。
    *   **模型分层**：简单问题路由给小模型（Llama 4 8B），复杂问题路由给大模型（Claude 4.6 Sonnet）。
*   **实现技巧**：Router 可以是一个专门微调过的分类小模型，也可以是基于 Embedding 的向量相似度匹配，只需极低的开销。

#### 3. 并行执行（Parallelization）

利用 LLM 并行处理能力，同时执行多个任务，提升速度或质量。包括两个变体：

*   **切片 (Sectioning)**: 将大任务拆解为独立的子任务并行处理。
...
*   **区别**：虽然子任务列表是动态的，但**执行模式是固定的**（例如：Split -> Map -> Reduce），依然属于静态编排；而 Autonomous Agent 可以自主决定是否循环、调用何种工具，流程拓扑是动态的。

#### 5. 评估者-优化者（Evaluator-Optimizer）

即经典的 **反思 (Reflection)** 模式。在一个循环流程中，引入“批评家”角色来提升质量。
...
当无法预定义路径时，需要让 LLM 接管控制权，进入 **自主编排 (Autonomous Orchestration)** 模式。Agent 不仅仅是调用工具，更是模拟这种**认知循环**。
...
为了解决 ReAct “走一步看一步”导致的短视问题，引入了显式的**规划 (Planning)** 阶段。
...
*   **层级制 (Hierarchical Crews)**: 
    *   类似于公司组织架构。
    *   **Manager Agent**: 负责拆解目标，监督进度，协调资源。
    *   **Specialist Agents**: 专注特定领域（如 DB设计专家、前端专家、QA专家）。
    *   *优势*: 权责分明，Manager 可以纠正下属的错误。

*   **协作制 (Joint Collaboration)**:
    *   类似于头脑风暴会议。
    *   多个智能体 (e.g., Developer + Tester) 处于同一个会话中，都能通过消息总线发言。
    *   通常需要一个 Speaker Selection 机制（谁下一个发言）。

### 9.1.3 模式选择指南

如何决定使用哪种模式？主要参考 **不确定性** 和 **复杂性** 两个维度。这对应了认知科学中的 **快思考 (System 1)** 与 **慢思考 (System 2)** 理论。

| 模式 | 认知模式 | 适用场景 | 复杂度 | 成本 |
| :--- | :--- | :--- | :--- | :--- |
| **提示词链** (Prompt Chaining) | 快思考 | 任务固定，步骤明确 | ⭐ | 💲 |
| **路由分发** (Routing) | 快思考 | 任务有几个明确变体 | ⭐⭐ | 💲 |
| **并行执行** (Parallelization) | 快思考 | 独立子任务，关注速度/覆盖面 | ⭐⭐ | 💲💲 |
| **编排者模式** (Orchestrator) | 混合态 | 动态子任务，需要统一协调 | ⭐⭐⭐ | 💲💲 |
| **评估优化** (Evaluator-Optimizer) | 慢思考 | 质量优先，容忍高延迟 | ⭐⭐⭐ | 💲💲💲 |
| **ReAct 智能体** (ReAct) | 慢思考 | 开放式探索，步骤未知 | ⭐⭐⭐⭐ | 💲💲💲 |
| **自主规划** (Plan-and-Solve) | 慢思考 | 极度复杂，长跨度任务 | ⭐⭐⭐⭐⭐ | 💲💲💲💲 |
...
#### Anthropic 的建议
Anthropic 在其最新的 Agent 指南中强烈推荐 **"使用简单、可组合的模式来构建有效的智能体（Building effective agents using simple, composable patterns）"**。他们发现，清晰定义的静态编排往往优于通用的自主编排。

#### OpenAI 的 Swarm 框架
OpenAI 推出的实验性框架 `Swarm` 专注于 **Handoffs (交接)** 模式。即智能体 A 完成工作后，将控制权完全移交给智能体 B（携带上下文），而不是层层嵌套的函数调用。这体现了**扁平化、模块化**的设计趋势。
