...
*   **优势**：极低的延迟，易于调试，每个节点可以使用不同规格的模型（例如大纲用 GPT-5，扩写用 GPT-5-mini）。
...
*   **适用场景**：
    ...
    *   **模型分层**：简单问题路由给小模型（Llama-4-Scout），复杂问题路由给大模型（Claude Opus 4.6）。
...
#### Anthropic 的建议
Anthropic 在其 Agent 指南中强烈推荐 **"使用简单、可组合的模式来构建有效的智能体（Building effective agents using simple, composable patterns）"**。他们发现，清晰定义的静态编排往往优于通用的自主编排。

#### OpenAI 的 Agents SDK
OpenAI 从早期的实验性框架 Swarm 演进而来，于 2025 年 3 月正式发布了 **Agents SDK**。该 SDK 将 **Handoffs（交接）** 作为核心原语之一——即智能体 A 完成工作后，将控制权完全移交给智能体 B（携带上下文），而不是层层嵌套的函数调用。SDK 还内置了 Guardrails（护栏）和 Tracing（追踪）等生产级特性，体现了**从实验到生产、扁平化、模块化**的设计趋势。

#### 开源社区的共识
2025 年 12 月，Linux 基金会成立了 **智能体 AI 基金会（Agentic AI Foundation, AAIF）**，致力于统一模型互操作性和工具标准。其中，Anthropic 主导的 **MCP（Model Context Protocol）** 正在成为智能体连接外部工具和数据源的行业标准协议（详见 [4.3 节](../04_tools/4.3_mcp.md)）。这些标准化努力使得上述设计模式可以更容易地跨框架复用。
