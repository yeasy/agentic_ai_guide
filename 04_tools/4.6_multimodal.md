...
多模态智能体的另一个关键维度是 **上下文窗口（Context Window）** 的扩展。2026 年，主流模型已支持 200K 乃至 1M+ tokens 的上下文窗口，这意味着智能体可以一次性接收远多于过去的材料，例如较大篇幅的书稿、长文档集合或大型项目中的关键文件集，而不再局限于少量片段。
...
class CodebaseAnalysisAgent:
    ...
    def __init__(self, repo_path: str):
        ...
        self.model = "claude-opus-4-6"  # 支持 1M tokens 级长上下文

    def analyze_entire_codebase(self, analysis_task: str) -> dict:
        """
        一次性分析整个代码库

        在 1M token 级模型中，可以一次性加载：
        - 中大型项目（50K-100K 行代码）
        - 所有配置文件和测试用例
        - 完整的文档
        """
