...
多模态智能体的另一个关键维度是 **上下文窗口（Context Window）** 的扩展。2026 年，主流模型已支持 200K 乃至 1M+ tokens 的上下文窗口，这意味着智能体可以一次性“看”完整个代码库、一本书或整个知识库，而不是只能处理片段。
...
|------|---------------------|-----------------|
| 单次对话 | 10-20 轮 | 数百轮 |
| 文档分析 | 最多 10-50 页 | 完整书籍（1000+ 页） |
| 代码库分析 | 单个文件 | 整个项目（100K+ 行） |
| 信息保持 | 前面信息逐渐“遗忘” | 完整上下文回忆 |

#### 代码库分析智能体

长上下文最具价值的应用之一是全代码库分析——将整个项目一次性送入模型进行安全审计、架构分析或重构建议：
...
class CodebaseAnalysisAgent:
    ...
    def __init__(self, repo_path: str):
        ...
        self.model = "claude-opus-4-6"  # 支持 200K+ tokens

    def analyze_entire_codebase(self, analysis_task: str) -> dict:
        """
        一次性分析整个代码库

        在 200K+ token 模型中，可以一次性加载：
        - 中大型项目（50K-100K 行代码）
        - 所有配置文件和测试用例
        - 完整的文档
        """

        all_code = self._collect_all_code()

        context = f"""
        项目结构：{self._get_project_structure()}

        代码库大小：{len(all_code)} 字符，约 {len(all_code) // 4} tokens
    ...
    def _collect_all_code(self) -> str:
        """收集所有源码文件"""
        all_code = []
    ...
    def _is_code_file(self, filename: str) -> bool:
        ...
        code_extensions = {
            ...
            ".rs", ".rb", ".php", ".sql", ".yaml", ".json",
            ".toml", ".ini", ".md", ".txt"
        }
        _, ext = os.path.splitext(filename)
        return ext.lower() in code_extensions

    def find_security_issues(self) -> dict:
        """全代码库安全审计"""
        return self.analyze_entire_codebase("""
        分析代码，找出以下安全问题：
        1. SQL 注入风险
        2. 硬编码的密钥或密码
        3. 未经授权的访问
        ...
        对每个问题提供：位置（文件名和行号）、严重程度、修复建议。
        """)
```

#### 知识库智能体

长上下文的另一个典型应用是构建知识库智能体——将整个知识库一次性加载，支持跨文档的综合分析和交叉引用：
...
class KnowledgeBaseAgent:
    ...
    def answer_with_full_kb(self, question: str) -> dict:
        """
        使用完整知识库回答问题
        ...
        all_knowledge = self._load_knowledge_base()
        ...
        return {
            ...
            "kb_size": f"{len(all_knowledge)} characters"
        }
...
长上下文虽然强大，但成本也不容忽视。以 Claude Opus 4.6 为例，输入 $5/百万 tokens，输出 $25/百万 tokens。处理 200K tokens 的请求，仅输入就需要 $1。以下是一个渐进式上下文管理策略，先用向量搜索缩小范围，不够再逐步扩展：

```python
class SmartContextManagement:
    """智能上下文管理——平衡成本与质量"""
    ...
    def answer_question_optimized(self, question: str) -> dict:
        """
        渐进式上下文策略：

        1. 先用向量搜索找 Top-5 相关文档（低成本）
        2. 尝试回答；如果信息不足，扩展到 Top-15
        3. 仍然不够时，加载完整知识库
        """

        # 第 1 步：向量搜索找最相关的文档
        relevant_docs = self._vector_search(question, top_k=5)
        short_context = self._format_docs(relevant_docs)
        ...
        # 第 2 步：信息不足时扩展搜索范围
        if "信息不足" in answer or "缺少" in answer:
            more_docs = self._vector_search(question, top_k=15)
            extended_context = self._format_docs(more_docs)
    ...
    def _vector_search(self, query: str, top_k: int = 5) -> list:
        """使用向量数据库搜索（如 Pinecone、Weaviate 等）"""
        # 实现向量搜索逻辑
        pass
...
这种渐进式策略在大多数情况下可以将成本降低 80% 以上——只有少数复杂问题才需要加载完整上下文。
