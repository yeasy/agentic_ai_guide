...
现代前沿模型（如 GPT-5.3、Claude 4.5、Gemini 2.0 Pro）都原生具备了极强的视觉能力。它们不再是将图片转成文字描述，而是直接**理解**像素。
...
图 4-17：LMM 视觉处理架构 (LMM Vision Processing Architecture)
...
图 4-18：视觉行动工作流程 (Visual Action Workflow)
...
**技术原理：视觉定位 (Visual Grounding)**

视觉行动的核心难点在于**定位（Grounding）**：如何让只懂文字的模型输出精确的 `(x, y)` 像素坐标？
...
在端到端模型中，音频被离散化为 **Audio Tokens**（通常使用 VQ-VAE 或 SoundStream 技术）。这意味着 1 秒钟的语音可能被表示为 25-50 个整数 ID。大模型可以像预测下一个文本 Token 一样，预测下一个音频 Token，从而直接生成语音。
...
graph TD
    ...
    subgraph Stack2 [端到端模式]
        E2E[原生语音模型<br>GPT-5.3/Gemini 2.0]
...
图 4-19：语音处理架构对比 (Comparison of Audio Processing Architectures)

**自动语音识别 (Automatic Speech Recognition, ASR)**

**OpenAI 开源的 Whisper 模型** 是目前的工业标准，支持多语言识别和翻译：
...
save(audio, "apology.mp3")
```

#### 2. 端到端实时对话

GPT-5.3 和 Gemini 2.0 Pro 展示了端到端的语音处理能力，相比传统流水线有巨大优势：
...
图 4-20：图像生成与编辑工作流 (Image Generation and Editing Workflow)
...
图 4-21：多模态智能体架构 (Multimodal Agent Architecture)
...
**下一节**: [4.7 智能体用户体验 (Agentic UX)](4.7_agentic_ux.md)