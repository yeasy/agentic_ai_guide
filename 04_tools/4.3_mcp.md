...
在实际部署中，仅靠文档描述安全原则远远不够。以下给出一套可直接复用的分层权限配置示例。

#### 1. 工具服务级权限声明

在工具服务的能力声明中，显式标注每个工具所需的最小权限范围（Scope），宿主在注册时据此做准入校验：
  ...
  "tools": [
    {
      ...
      "permissions": {
        ...
        "denied_operations": ["DROP", "DELETE", "UPDATE", "INSERT", "ALTER"]
      }
    },
    {
      "name": "write_file",
      "description": "向指定沙箱目录写入文件",
      "inputSchema": {
        "type": "object",
        "properties": {
          "path": { "type": "string" },
      ...
      "permissions": {
        ...
        "sandbox_root": "/workspace/agent-output",
...
在宿主（Host）层实施硬性拦截，确保工具服务返回的参数不会突破沙箱边界：
...
class SandboxGuard:
    """宿主侧沙箱守卫：在工具调用前拦截非法路径"""

    def __init__(self, sandbox_root: str, allowed_extensions: list[str]):
    ...
    def validate_path(self, raw_path: str) -> Path:
        """
        验证并规范化路径，防止目录遍历攻击。
        失败时抛出 PermissionError 而非返回错误码，确保调用链硬中断。
        """
        # 1. 规范化：消除 ../ ./ 和符号链接
        ...
        if not str(resolved).startswith(str(self.sandbox_root)):
            raise PermissionError(
                f"路径越界拦截: '{raw_path}' 解析为 '{resolved}'，"
                f"超出沙箱 '{self.sandbox_root}'"
            )

        # 3. 扩展名白名单
        if resolved.suffix not in self.allowed_extensions:
            raise PermissionError(
                f"文件类型拒绝: '{resolved.suffix}' 不在允许列表 {self.allowed_extensions}"
            )

        return resolved

    def validate_sql(self, sql: str, denied_ops: list[str]) -> str:
        """SQL 参数清洗：拦截危险操作"""
        sql_upper = sql.upper().strip()
        for op in denied_ops:
            if op in sql_upper:
                raise PermissionError(f"SQL 操作拒绝: 检测到禁止的操作 '{op}'")
        return sql
...
safe_path = guard.validate_path("reports/summary.json")

# ❌ 目录遍历攻击 → 抛出 PermissionError

# guard.validate_path("../../etc/passwd")

# ❌ 危险 SQL → 抛出 PermissionError

# guard.validate_sql("DROP TABLE users;", ["DROP", "DELETE"])

```

#### 3. 企业级 IAM 策略模板

对于需要接入企业身份管理系统的场景，建议为每个智能体会话创建临时的最小权限凭证：

```yaml

# agent-iam-policy.yaml — 智能体会话的最小权限模板

apiVersion: iam/v1
kind: AgentSessionPolicy
metadata:
  name: readonly-analyst-agent
  labels:
...
spec:
  ...
  session:
    ...
    max_tool_calls: 200         # 单会话工具调用上限
    max_tokens_budget: 100000   # Token 预算

  # 数据库权限
  database:
    - resource: "db://analytics/*"
      actions: ["SELECT"]        # 仅允许 SELECT
      row_limit: 10000
      denied_tables: ["pii_*", "credentials_*"]

  # 文件系统权限
  filesystem:
    - resource: "/workspace/agent-output/**"
      actions: ["read", "write"]
...
> **提示**
> **实施路径建议**：对于中小团队，可以先从路径沙箱守卫（Guard）+ SQL 白名单起步，不必一步到位搭建完整 IAM 策略引擎。随着业务规模扩大，再逐步引入会话级 Token 预算、网络域白名单和审计日志。关于速率控制与熔断防御的详细配置，参见[第 9 章 AgentOps](../09_agentops/README.md)。

### 4.3.7 代理支付协议：从工具调用到自主交易

MCP 解决了“智能体如何发现和调用工具”的问题，但当工具或服务需要**付费**才能使用时，还需要一层支付协议来闭合交易环路。

2026 年 3 月，Stripe 发布了 **Machine Payments Protocol（MPP）**，这是一个面向 AI 代理自主交易的开放标准。其核心流程与 MCP 的工具调用模式高度互补：

```text
代理请求资源   →   服务返回支付请求（金额、币种、收款地址）
     ↓                         ↓
代理授权支付   ←   服务验证到账后返回资源
```

MPP 支持多种支付方式：稳定币（链上结算）、法币（卡支付、先买后付）以及 **Shared Payment Tokens（SPT）**。开发者可通过 Stripe 的 PaymentIntents API 在几行代码内接入，交易在 Stripe 仪表板中与人类支付统一管理。

MPP 与 MCP 的关系可以类比为“工具发现层”与“价值交换层”的分工：MCP 让代理知道有哪些服务可用以及如何调用，MPP 让代理能够为这些服务自主付费。两者结合，使得代理从“只能调用免费工具”进化为“可以在开放市场上按需购买服务的经济主体”。

MPP 属于 Stripe 更广泛的 **Agentic Commerce Suite** 的一部分，该套件还包括 Agentic Commerce Protocol（ACP）等组件，旨在为代理经济提供完整的商业基础设施。
