...
Agent Skills 是 Anthropic 主导的开放格式规范，用于扩展 AI Agent 的能力和专业知识。它提供了一种轻量级、可移植的方式来封装领域知识和工作流程。
...
1. **Discovery（发现）**：启动时仅加载每个 Skill 的 name 和 description，足以判断其相关性
2. **Activation（激活）**：当任务匹配某个 Skill 的描述时，将完整 SKILL.md 指令加载到上下文
3. **Execution（执行）**：Agent 按照指令执行，按需加载引用的文件或执行捆绑的脚本
