...
**智能体技能（Agent Skills）** 是 Anthropic 主导的开放格式规范，用于扩展 AI Agent 的能力和专业知识。它提供了一种轻量级、可移植的方式来封装领域知识和工作流程。

### 4.4.1 什么是 Agent Skills

Skills 是一种简单的开放格式，用于赋予 Agent 新的能力和专业知识。与 MCP 提供系统连接能力不同，Skills 专注于**过程性知识**的封装。

> MCP 是**通道**，Skills 是**智慧**。
> 
> MCP 解决"通过什么做"，Skills 解决"如何做"。

#### Skills 的核心价值

- **领域专业知识**：将法律审查流程、数据分析管道等专业知识封装为可复用指令
- **新能力扩展**：赋予 Agent 创建演示文稿、构建 MCP 服务器、分析数据集等能力
- **可重复工作流**：将多步骤任务转化为一致、可审计的工作流程
- **跨平台互操作**：在不同 Skills 兼容的 Agent 产品间复用相同技能
...
```mermaid
flowchart LR
    a["1. Discovery"] --> b["2. Activation"] --> c["3. Execution"]
    
    subgraph discovery ["启动阶段"]
        a
        d["加载 name + description"]
    end
    ...
    subgraph execution ["执行阶段"]
        c
        f["按需加载 scripts/references/assets"]
    end
```

1. **发现（Discovery）**：启动时仅加载每个 Skill 的 name 和 description，足以判断其相关性
2. **激活（Activation）**：当任务匹配某个 Skill 的描述时，将完整 SKILL.md 指令加载到上下文
3. **执行（Execution）**：Agent 按照指令执行，按需加载引用的文件或执行捆绑的脚本
...
| 层级 | 内容 | Token 预算 | 加载时机 |
|------|------|-----------|---------|
| Metadata | name + description | ~100 tokens | 启动时加载所有 Skills |
| Instructions | 完整 SKILL.md 正文 | <5000 tokens（建议） | 任务匹配时激活 |
| Resources | scripts/、references/、assets/ | 按需 | 执行时按需加载 |
...
Skills 和 MCP 是互补的：Skills 定义"做什么"和"怎么做"，MCP 提供"用什么工具做"。
...
│           ▼         ▼         ▼         ▼                   │
│      ┌────────┐┌────────┐┌────────┐┌────────┐               │
│      │Salesforce││ Slack  ││ Google ││ Notion │               │
│      │  MCP   ││  MCP   ││ Drive  ││  MCP   │               │
│      └────────┘└────────┘└────────┘└────────┘               │
└─────────────────────────────────────────────────────────────┘
```
...
- **避免指令冲突**：如果 MCP 指定返回 JSON，而 Skill 要求 Markdown 表格，Agent 需要猜测。让 MCP 处理连接，Skill 处理呈现
- **保持复用性**：一个 MCP 服务器可支持多个 Skills；一个 Skill 可编排多个 MCP 服务器
- **利用渐进式加载**：将详细参考资料放在 references/ 目录，仅在需要时加载

#### 验证工具

使用官方 skills-ref 工具验证 Skill 格式：
...
Agent Skills 为 AI Agent 能力扩展提供了标准化的解决方案：

1. **轻量级格式**：基于 Markdown，易于编写和维护
2. **渐进式加载**：优化上下文使用，仅在需要时加载
3. **与 MCP 互补**：Skills 提供"智慧"，MCP 提供"通道"
4. **开放生态**：可跨平台复用，社区共享
...
**下一节**: [环境交互：代码解释器与浏览器操作](4.5_environment.md)
